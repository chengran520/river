var showRelease = {
	...
	showModal : function(type, isMsg) {
		...
		setTimeout(function() {
			if (!isMsg) {
				...
				//初始化上传文件
				formUtil.uploadFile("myFile", WEB_ROOT + "news/addFileUpload",1);
				$("#myFile").on("fileuploaded", function(e, data) {
	...
	querFileTable : function() {
		...
		bootstrapTable.columns = [ {
			...
			formatter : function(value, row, index) {
				var pageSize = $('#table_fileManage').bootstrapTable('getOptions').pageSize;//通过表的#id 可以得到每页多少条
	            var pageNumber = $('#table_fileManage').bootstrapTable('getOptions').pageNumber;//通过表的#id 可以得到当前第几页
	...
	queryMsgTable : function() {
		...
		bootstrapTable.columns = [ {
			...
			width : 1,
			formatter : function(value, row, index) {
				var pageSize = $('#table_showRelease').bootstrapTable('getOptions').pageSize;//通过表的#id 可以得到每页多少条
	            var pageNumber = $('#table_showRelease').bootstrapTable('getOptions').pageNumber;//通过表的#id 可以得到当前第几页
	            return pageSize * (pageNumber - 1) + index + 1;//返回每条的序号： 每页条数 * （当前页 - 1 ）+ 序号
			}
		}, {
			field : 'msgTitle',
